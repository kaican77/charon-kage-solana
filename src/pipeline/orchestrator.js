...
export async function processCandidateFromSignals(signals) {
  ...
  if (selectedRow && boolSetting('agent_enabled', true) && batchDecision.verdict === 'BUY' && batchDecision.confidence >= (activeStrategy().llm_min_confidence ?? numSetting('llm_min_confidence', 75))) {
    if (!canOpenMorePositions()) {
      const max = numSetting('max_open_positions', 3);
    ...
    logDecisionEvent({
      ...
      guardrails: {
        ...
        confidenceThreshold: activeStrategy().llm_min_confidence ?? numSetting('llm_min_confidence', 75),
