...
export const numericFilterLabels = {
  ...
  trending_max_top10_holder_rate: 'maximum top-10 holder rate (0.25 = 25%)',
  trending_max_dev_hold_rate: 'maximum dev hold rate (0 = dev must hold 0%)',
  trending_max_sniper_hold_rate: 'maximum sniper hold rate (0.2 = 20%)',
  min_smart_degen_count: 'minimum smart-money wallets (0 = off)',
  min_renowned_count: 'minimum KOL wallets (0 = off)',
  max_rsi_14: 'maximum RSI 14 (0 = off)',
};
...
export function filtersKeyboard() {
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: '⚙️ Configure in Strategy', callback_data: 'menu:strategy' }],
        [
...
export function agentText() {
  ...
  return [
    ...
    `${modeIcon} Mode: <b>${escapeHtml(mode)}</b>`,
    `🧠 LLM: <b>${strat.use_llm && ENABLE_LLM && LLM_API_KEY ? '✅ configured' : '❌ disabled'}</b>`,
    `🎯 Confidence: ${fmtPct(strat.llm_min_confidence)}`,
    `📍 Open positions: ${openPositionCount()}/${strat.max_open_positions || 'unlimited'}`,
    `📦 Batch candidates: ${strat.llm_candidate_pick_count || numSetting('llm_candidate_pick_count', 10)}`,
  ...
  return {
    reply_markup: {
      inline_keyboard: [
        ...
        [
          { text: 'Conf 50%', callback_data: 'stratcfg:llm_min_confidence:50' },
          { text: 'Conf 70%', callback_data: 'stratcfg:llm_min_confidence:70' },
          { text: 'Conf 80%', callback_data: 'stratcfg:llm_min_confidence:80' },
        ],
        [
          { text: 'Batch 5', callback_data: 'stratcfg:llm_candidate_pick_count:5' },
          { text: 'Batch 10', callback_data: 'stratcfg:llm_candidate_pick_count:10' },
        ],
        [
...
export function mainMenuText() {
  ...
  return [
    '🛶 <b>CHARON</b>',
    divider(),
    `${modeIcon} <b>${escapeHtml(mode.toUpperCase())}</b> · 🤖 Agent: <b>${boolSetting('agent_enabled', true) ? 'on' : 'off'}</b>`,
    `🎯 Strategy: <b>${escapeHtml(strat.name)}</b>`,
    `📍 Positions: ${openPositionCount()}/${strat.max_open_positions || '∞'}`,
    '',
    'Pick a menu below 👇',
  ].join('\n');
}
...
export function strategyMenuText() {
  ...
  return [
    '🎯 <b>CHARON · STRATEGY</b>',
    divider(),
    `▶ Active: <b>${STRAT_ICONS[strat.id] || '🎯'} ${escapeHtml(strat.name)}</b>`,
    `${entryIcons[strat.entry_mode] || '?'} Entry: ${strat.entry_mode}`,
    `🔢 Min sources: ${strat.min_source_count}`,
    `💱 Fee required: ${strat.require_fee_claim ? '✅ yes' : '❌ no'}`,
    `💵 Size: ${fmtSol(strat.position_size_sol)} SOL`,
    `🎯 TP/SL: ${fmtPct(strat.tp_percent)} / ${fmtPct(strat.sl_percent)}`,
    ...
    strat.max_ath_distance_pct < 0 ? `⛰ Max ATH distance: ${strat.max_ath_distance_pct}%` : null,
    strat.trending_max_top10_holder_rate > 0 ? `📊 Top10 holders ≤ ${fmtPct(strat.trending_max_top10_holder_rate * 100)}` : null,
    strat.trending_max_dev_hold_rate != null ? `👨💻 Dev hold ≤ ${fmtPct(strat.trending_max_dev_hold_rate * 100)}` : null,
    strat.trending_max_sniper_hold_rate > 0 ? `🎯 Sniper hold ≤ ${fmtPct(strat.trending_max_sniper_hold_rate * 100)}` : null,
    strat.trending_require_dex_paid ? '💳 Dex Paid: wajib' : null,
...
export function strategyKeyboard() {
  ...
  const selector = all.map(s => [{
    text: `${s.enabled ? '▶ ' : ''}${STRAT_ICONS[s.id] || '🎯'} ${s.name}`,
  ...
  const config = [
    ...
    [
      { text: `Top10 ${fmtPct((strat.trending_max_top10_holder_rate ?? 0.25) * 100)}`, callback_data: 'stratinput:trending_max_top10_holder_rate' },
      { text: `Dev ${fmtPct((strat.trending_max_dev_hold_rate ?? 0) * 100)}`, callback_data: 'stratinput:trending_max_dev_hold_rate' },
      { text: `Sniper ${fmtPct((strat.trending_max_sniper_hold_rate ?? 0.2) * 100)}`, callback_data: 'stratinput:trending_max_sniper_hold_rate' },
    ],
    [
      { text: `Dex Paid ${strat.trending_require_dex_paid ? '✅' : '❌'}`, callback_data: 'stratcfg:trending_require_dex_paid' },
    ],
    [
  ...
  return {
    reply_markup: {
      inline_keyboard: [
        ...
        ...selector,
        [{ text: '⚙️ ── Configure ──', callback_data: 'noop' }],
        ...config,
        [{ text: '⬅️ Back', callback_data: 'menu:main' }],
      ],
    },
